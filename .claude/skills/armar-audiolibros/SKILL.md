...
## Resuelto 24-jul-2026 (además de "El día de ayudar a todos")
- **`futbol/gran-torneo` ("Los Grandes Juegos") — CORREGIDO Y REGENERADO**: causa raíz distinta a las demás — `ia_maestra.png` de futbol no tiene NINGÚN personaje humano (solo objetos con carita: pelota, trofeo, botín), por eso el motor dibujaba objetos en vez de un equipo. Se agregó `ELENCO_FIJO[("futbol","gran-torneo")]` con 4 roles fijos (capitán, más rápida, más fuerte, más chiquita) + `protagonista_extra` (nuevo campo de `ELENCO_FIJO`: agrega vestimenta fija al protagonista cuando el combo lo pide, ej. "la misma camiseta de local que sus compañeros" — antes solo existía para roles secundarios). Rival con camiseta de otro color. Regenerado nene+nena completo, 0 rechazos QA, revisado visualmente: consistente.
- **Corrección de género en el TEXTO narrado**: "el más chiquito"/"del más chiquito" pasó a "la más chiquita" en `ayudar-a-todos` Y en `gran-torneo` (libro_historias.py) — el personaje ilustrado en ambos combos ya armados es una nena; Pablo lo encontró al escuchar/leer el audiolibro ya regenerado (no alcanza con mirar las imágenes, hay que releer el texto que dice el narrador).
- **Portada de "Mi biblioteca" con caché vieja (Cloudflare 24h)**: al regenerar un audiolibro ya cacheado, la card de la biblioteca del cliente seguía mostrando `pag_00.jpg` VIEJO aunque el visor ya tuviera el libro corregido — Cloudflare cachea esa imagen 24hs y la URL no cambiaba. Fix en `/opt/ct3d/backend/tienda_clientes.py` (`_cover_de`): para `tipo=="libro-audio"` ahora agrega `?v=<manifest.creado>` (lee el timestamp de regeneración del manifest del motor, mismo host) — mismo mecanismo que ya usaban rompecabezas-web/actividades-web para este mismo problema. **Acordarse**: después de regenerar CUALQUIER audiolibro ya vendido/cacheado, además de rehacer el arte hay que volver a llamar `audiolibro.crear(...)` con el MISMO token para que el token ya entregado (preview o venta real) muestre el fix — regenerar el cache de `catalogo_arte/` solo no alcanza, ese token ya tiene sus jpg/mp3 renderizados una vez y no se actualizan solos.

## Pendiente (backlog de consistencia — no regenerado todavía)
- `artistas/pequeno-maestro` ("Pequeños artistas") — **confirmado visualmente, variante nueva del bug**: `ia_maestra.png` de artistas ya trae 2 personajes fijos (nena de colitas + nene de guardapolvo azul) que son "los personajes del tema", NO el protagonista. En las páginas donde la historia pide un protagonista genérico (de espaldas, cara nunca visible) + un "personaje nuevo chiquito y tímido" aparte, el arte generado solo repite esos 2 personajes fijos de siempre — el protagonista no aparece por separado y el rol "nuevo/chiquito" termina siendo la nena de la referencia en vez de un chico distinto. Mismo fix que `ayudar-a-todos`/`gran-torneo`: un `ELENCO_FIJO` para artistas (los 2 de referencia) + dejar que la cadena de anclas (protagonista/elenco) haga de la actual escena en curso un personaje realmente aparte del elenco fijo.
- `monstruos/aventura` ("La invitación mágica"): pijama del protagonista cambia de color y de manga/pantalón corto-largo entre páginas — **confirmado visualmente** (verde pág. 2 → azul pág. 3, mismo combo).
- `aviadores/gran-viaje` ("La entrega importante", tema aviadores): según Pablo el protagonista pierde el traje de aviador/aviadora en varias páginas del medio del libro (no confirmado en el detalle, se ve consistente en las primeras páginas revisadas). Pablo sugiere fijar el género en NENA (ya predomina en el arte existente) para minimizar cuántas páginas hay que regenerar.
- `tesoro` ("El mapa del tesoro"): según Pablo, en el combo que revisó aparecen personajes de más que la escena no pide, y el protagonista debería llevar un uniforme fijo (bombero, según su reporte) que no se sostiene en todas las páginas, y el nene rescatado cambia de ropa entre páginas cuando debería ser la misma (envuelto en la frazada roja al final). OJO al retomar: el combo que Pablo miraba no es necesariamente `campamento/tesoro` (el único cacheado hoy en el repo, y que a simple vista SÍ es consistente) — puede ser un combo `bomberos/tesoro` generado en una venta puntual; ubicarlo primero (o pedirle el link a Pablo) antes de tocar nada.
