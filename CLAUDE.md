...
- **"Elegí tu aventura" — PROTOTIPO** (`aventura.py` + `aventura_web.py`, 11-jul-2026): a diferencia de `libro.py` (100% lineal), acá el chico ELIGE el camino en un grafo de nodos con decisiones (por ahora solo tema `safari`: 3 puntos de decisión, 2 finales), entregado como link vivo `/leer/<token>/` reusando el arte ya generado del libro de cuento (`temas/<tema>/overrides/libro/2..8.png` — sin ilustración IA nueva todavía). Player en `aventura_player.html/.js`. Sin integración a tienda ni ilustración propia por rama: es solo para validar el mecanismo antes de invertir en más temas/contenido.
