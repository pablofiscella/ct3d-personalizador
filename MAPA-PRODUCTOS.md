...
- **Imprimible (PDF):** `libro.py` — arma las páginas. **OJO:** su contenido (ambientación por tema) está DENTRO del mismo archivo: `HISTORIAS`, `ARGUMENTOS`, `ARGUMENTOS_EXT` (`libro.py:77/247/376`).
- **Web narrado (audiolibro):** `audiolibro.py` — ruta `/al/<token>` (`servicio.py:858`).
- **Contenido compartido:** `libro_historias.py` — catálogo de historias del audiolibro: `ARGUMENTOS_LARGO`, `TITULOS`, `CORTO_IDX`. Lo usan `audiolibro.py`, `libro_ia.py`, `libro.py`.
- **Arte IA:** `libro_ia.py`.
- ➜ "¿Dónde está la historia?" tiene HOY dos respuestas: `libro_historias.py` (audiolibro) y `libro.py` (ambientación del libro-kit). Ver Inconsistencias #2.
...
2. **Historias en dos lados:** `libro_historias.py` (audiolibro) + `libro.py` HISTORIAS/ARGUMENTOS (libro-kit).
